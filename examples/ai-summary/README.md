...
`summarize.js` show how to send the prompt with plain `fetch` to an
OpenAI-compatible endpoint or to Anthropic.
...
```bash
node summarize.js https://x.com/jack/status/20
node summarize.js https://x.com/jack/status/20 > prompt.txt   # status line goes to stderr
```
